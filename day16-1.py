...
with open('data/day16.txt', 'r') as file:
    for i in range(20):
        ticket_rules.append(file.readline().strip('\n'))
...
for rule in ticket_rules:
    ...
    range_1 = split[-3].split('-')
    range_2 = split[-1].split('-')

    val_1_low = int(range_1[0])
    val_1_hi = int(range_1[1])

    val_2_low = int(range_2[0])
    val_2_hi = int(range_2[1])

    for i in range(val_1_low, val_1_hi + 1):
        valid_numbers[i] = 1
    
    for i in range(val_2_low, val_2_hi + 1):
        valid_numbers[i] = 1


def is_valid_value(num):
    return valid_numbers.get(num, 0)
